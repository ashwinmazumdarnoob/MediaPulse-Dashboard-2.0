...
import {
  ...
  Columns3, LayoutDashboard, TrendingUp, TrendingDown, Copy, Info,
  Eye, Table2, Zap, Database, FileBarChart
} from "lucide-react";
...
const C = {
  ...
  border:"#253050",
  p:"#7c5cfc", pL:"#a78bfa", pBg:"rgba(124,92,252,0.12)",
...
/* ═══════════════════════════════════════════════════════════
   ...
   DV360: HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_TAMILNADU_AWA_INNS-CTV
   Pos:   [0]     [1]    [2]   [3]   [4] [5] [6] [7...]      [obj] [placement]
   AF:    HM_SWIGGY_SWIM_NOICE_BRN_FBIG_SOCIAL_LC_Geos_RU_DATE_ASSET
   ═══════════════════════════════════════════════════════════ */
const LANG_MAP = {ENG:"English",HIN:"Hindi",TAM:"Tamil",TEL:"Telugu",MAL:"Malayalam",KAN:"Kannada",BEN:"Bengali",MAR:"Marathi"};
const KNOWN_PLATFORMS = ["DV360","META","FBIG","FB","GOOGLE","GAD","MEDIASMART","MS","PROG","PROGRAMMATIC","TWITTER","LINKEDIN"];
...
function parseCampaignName(campaign, creative) {
  ...
  const isDV360 = parts.length >= 8 && /^HM$/i.test(parts[0]) && KNOWN_PLATFORMS.includes(p2);

  if (isDV360) {
    // Platform
    if (p2 === "DV360") r.Platform = "DV360";
    else if (p2 === "FBIG" || p2 === "FB" || p2 === "META") r.Platform = "Meta";
    else if (p2 === "GOOGLE" || p2 === "GAD") r.Platform = "Google";
    else if (p2 === "MEDIASMART" || p2 === "MS") r.Platform = "Mediasmart";
    else if (p2 === "PROG" || p2 === "PROGRAMMATIC") r.Platform = "Programmatic";
    else r.Platform = p2;

    // Asset Type
    const a3 = (parts[3] || "").toUpperCase();
    r.AssetType = a3 === "VIDEO" ? "Video" : a3 === "DISPLAY" ? "Display" : a3 === "SOCIAL" ? "Social" : a3 || "Other";
    ...
    // Geo: between NU/RU and AWA/PERF/CONV
    ...
    else if (/CTV/i.test(last)) r.Placement = "CTV";
    else r.Placement = last;
  }
  // AppsFlyer: HM_SWIGGY_SWIM_NOICE_BRN_FBIG_SOCIAL_LC_Geos_RU_...
  else if (/FBIG|SWIM/i.test(c)) {
    r.Platform = /FBIG/i.test(c) ? "Meta" : "Other";
    r.AssetType = /VIDEO.*DISPLAY/i.test(c) ? "Video & Display" : /VIDEO/i.test(c) ? "Video" : /DISPLAY/i.test(c) ? "Display" : "Social";
    for (const p of parts) { const pu = p.toUpperCase().trim(); if (pu === "NU") { r.UserType = "New User"; break; } if (pu === "RU") { r.UserType = "Returning User"; break; } }
    ...
    if (lcIdx >= 0 && ruIdx > lcIdx) {
      const gs = parts.slice(lcIdx + 1, ruIdx).join(" ");
      r.Geo = /T2|Tier/i.test(gs) ? "Tier 2 Cities" : /Mum|Del|NCR|Hyd|Blr|Che|Pun/i.test(gs) ? "Metro Cities" : cleanGeo(gs);
    }
    r.Placement = "Social";
  }

  // Language from creative: ..._TAM, ..._HIN, ..._ENG
  if (cr) {
    const lastCr = (cr.split("_").pop() || "").toUpperCase().trim();
    if (LANG_MAP[lastCr]) r.Language = LANG_MAP[lastCr];
    if (/INNS/i.test(cr)) r.AdFormat = "In-Stream";
    else if (/SHORTS/i.test(cr)) r.AdFormat = "Shorts";
    else if (/SKIP/i.test(cr)) r.AdFormat = "Skippable";
    else if (/BUMPER/i.test(cr)) r.AdFormat = "Bumper";
  } else {
    r.AdFormat = r.Placement.includes("In-Stream") ? "In-Stream" : r.Placement.includes("Shorts") ? "Shorts" : "Other";
  }
  return r;
}

/* ═══════════════════════════════════════════════════════════
   COLUMN ALIASES + AUTO-MAP
   ═══════════════════════════════════════════════════════════ */
const ALIASES = {
  ...
  "Add to Cart": ["event_count_instamart_add_to_cart","event_count_im_add_to_cart"],
  "First Order": ["event_count_instamart_first_order","event_count_im_first_order_purchase"],
  Purchase: ["event_count_instamart_purchase","event_count_im_purchase"],
};
...
function autoMap(headers) {
  ...
  for (const k of Object.keys(ALIASES)) {
    for (const h of headers) { if (h.toLowerCase().trim() === k.toLowerCase()) { m[k] = h; break; } }
  }
  // Pass 2: alias match
  for (const [k, al] of Object.entries(ALIASES)) {
    if (m[k]) continue;
    for (const h of headers) { if (al.includes(h.toLowerCase().trim())) { m[k] = h; break; } }
  }
  return m;
}
...
/* ═══════════════════════════════════════════════════════════
   PARSE UTILITIES
   ═══════════════════════════════════════════════════════════ */
function isDV360Footer(row) {
  const first = String(Object.values(row)[0] || "").toLowerCase().trim();
  return /^(date range|group by|filter by|mrc accredited|reporting numbers)/.test(first) || (first === "" && Object.values(row).filter(x => String(x || "").trim()).length <= 1);
}
...
function parseDate(v) {
  ...
  const d = new Date(s);
  return (!isNaN(d) && d.getFullYear() > 2000) ? d.toISOString().slice(0, 10) : s;
}
...
function parseFile(file) {
  return new Promise((res, rej) => {
    ...
    if (!["csv", "tsv", "xlsx", "xls"].includes(ext)) { rej(new Error(`Unsupported: .${ext}`)); return; }
    if (ext === "csv" || ext === "tsv") {
      Papa.parse(file, {
        header: true, skipEmptyLines: true, dynamicTyping: false,
        complete: r => { if (!r.data?.length) { rej(new Error("No data")); return; } res({ headers: r.meta.fields || Object.keys(r.data[0]), rows: r.data }); },
        error: e => rej(new Error(e.message)),
      });
    } else {
      const rd = new FileReader();
      rd.onload = e => { try { const wb = XLSX.read(e.target.result, { type: "array", cellDates: true }); const data = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval: "" }); if (!data.length) { rej(new Error("Empty")); return; } res({ headers: Object.keys(data[0]), rows: data }); } catch (e) { rej(new Error(e.message)); } };
...
function validate(rows, cm, type) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Parse campaign name → 7 dimensions
...
function aggByDim(data, dimKey) {
  ...
  data.forEach(r => {
    ...
    g["Complete Views"] += r["Complete Views"] || 0; g.Sessions += r.Sessions || 0; g.Installs += r.Installs || 0;
  });
...
function KpiCard({ label, value, sub, color }) {
  return (
    <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: "16px 18px", borderTop: `3px solid ${color || C.p}`, flex: 1, minWidth: 130 }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: C.t2, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 21, fontWeight: 700, color: C.t0, marginBottom: 3 }}>{value}</div>
      {sub && <div style={{ fontSize: 10, color: C.t1 }}>{sub}</div>}
    </div>
  );
}
...
function AlertBox({ type, items, title }) {
  ...
  const isE = type === "error", col = isE ? C.r : C.a;
  return (
    <div style={{ background: col + "12", border: `1px solid ${col}40`, borderRadius: 10, padding: 16, marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        {isE ? <AlertTriangle size={16} color={col} /> : <Info size={16} color={col} />}
        <span style={{ fontWeight: 600, color: col, fontSize: 13 }}>{title}</span>
      </div>
      {items.map((m, i) => <div key={i} style={{ fontSize: 12, color: C.t1, paddingLeft: 24, marginTop: 4 }}>{m}</div>)}
    </div>
  );
}

function DimBreakdown({ data, title, metricKey, formatter }) {
    ...
    <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
      ...
      <ResponsiveContainer width="100%" height={Math.max(160, data.length * 32 + 40)}>
        <BarChart data={data.slice(0, 10)} layout="vertical" margin={{ left: 10, right: 20 }}>
          ...
          <Bar dataKey={metricKey} radius={[0, 4, 4, 0]} barSize={16}>
            {data.slice(0, 10).map((_, i) => <Cell key={i} fill={CLR[i % CLR.length]} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
    ...
    <div style={{ width: 210, background: C.bg1, borderRight: `1px solid ${C.border}`, padding: "20px 0", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 50, overflowY: "auto" }}>
      <div style={{ padding: "0 20px 24px", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg,${C.p},${C.b})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Database size={16} color="#fff" />
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: C.t0 }}>MediaPulse</div>
            <div style={{ fontSize: 10, color: C.t2, letterSpacing: 1, textTransform: "uppercase" }}>Analytics Engine</div>
          </div>
        </div>
      </div>
      <div style={{ padding: "12px 0", flex: 1 }}>
        {items.map(({ id, label, Icon, step }) => {
          const ok = step <= maxStep, act = active === id;
          return <button key={id} onClick={() => ok && goTo(id)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "11px 20px", border: "none", background: act ? C.pBg : "transparent", color: act ? C.pL : ok ? C.t1 : C.t2 + "60", cursor: ok ? "pointer" : "not-allowed", fontFamily: C.font, fontSize: 13, fontWeight: act ? 600 : 400, borderLeft: act ? `3px solid ${C.p}` : "3px solid transparent", opacity: ok ? 1 : .35, textAlign: "left" }}><Icon size={16} />{label}</button>;
        })}
      </div>
      <div style={{ padding: "16px 20px", borderTop: `1px solid ${C.border}`, fontSize: 11, color: C.t2 }}>v2.0 · Browser Only</div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); onFile(e.dataTransfer.files[0]); }} onClick={() => ref1.current?.click()} style={{ border: `2px dashed ${drag ? C.p : C.border}`, borderRadius: 14, padding: "48px 32px", textAlign: "center", cursor: "pointer", background: drag ? C.pBg : C.bg2 }}>
          ...
          <div style={{ fontSize: 12, color: C.t2, marginBottom: 12 }}>DV360, Meta, Google, Programmatic</div>
          <div style={{ display: "inline-block", background: C.p, color: "#fff", borderRadius: 6, padding: "6px 16px", fontSize: 12, fontWeight: 600 }}>CSV or Excel</div>
        ...
        <div onClick={() => ref2.current?.click()} style={{ border: `2px dashed ${C.border}`, borderRadius: 14, padding: "48px 32px", textAlign: "center", cursor: "pointer", background: C.bg2 }}>
          ...
          <div style={{ fontSize: 12, color: C.t2, marginBottom: 12 }}>Attribution, installs, events</div>
          <div style={{ display: "inline-block", background: C.cy + "30", color: C.cy, borderRadius: 6, padding: "6px 16px", fontSize: 12, fontWeight: 600 }}>CSV or Excel</div>
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <button onClick={onDemo} style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 28px", color: C.t0, cursor: "pointer", fontFamily: C.font, fontSize: 13, fontWeight: 500, display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      {rawRows && rawRows.length > 0 && (() => {
        const hdrs = Object.keys(rawRows[0]).slice(0, 8);
        return (
          <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginTop: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}><Eye size={14} color={C.pL} /><span style={{ fontWeight: 600, fontSize: 13, color: C.t0 }}>Preview ({rawRows.length} rows)</span></div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead><tr>{hdrs.map(h => <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 10, fontWeight: 600, color: C.t2, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap", textTransform: "uppercase" }}>{h.length > 20 ? h.slice(0, 20) + "…" : h}</th>)}</tr></thead>
                <tbody>{rawRows.slice(0, 4).map((r, i) => <tr key={i} style={{ background: i % 2 ? C.bg1 + "80" : "transparent" }}>{hdrs.map(h => <td key={h} style={{ padding: "7px 12px", color: C.t1, borderBottom: `1px solid ${C.border}15`, whiteSpace: "nowrap", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis" }}>{String(r[h] ?? "").slice(0, 25)}</td>)}</tr>)}</tbody>
              </table>
            </div>
            <div style={{ textAlign: "right", marginTop: 14 }}>
              <button onClick={onGoMapping} style={{ background: C.p, color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", cursor: "pointer", fontFamily: C.font, fontSize: 14, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      {!rawRows && (
        <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginTop: 8 }}>
          ...
          <div style={{ fontSize: 12, color: C.t1, lineHeight: 1.8 }}>
            <strong style={{ color: C.t0 }}>DV360:</strong> Insertion Order, Revenue (Adv Currency), Impressions, Clicks, Complete Views — footer auto-stripped<br />
            <strong style={{ color: C.t0 }}>Meta / Google:</strong> Standard exports with Spend, Impressions, Clicks<br />
            <strong style={{ color: C.t0 }}>AppsFlyer:</strong> Overview reports — comma-formatted numbers OK<br />
            <strong style={{ color: C.t0 }}>Auto-parsed from names:</strong> Platform, Geo, Language, User Type, Placement, Asset Type, Ad Format
          </div>
        </div>
      )}
    </div>
...
function MappingPage({ headers, colMap, setColMap, onLaunch, valResult, platforms }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: C.t2, marginBottom: 12, fontSize: 14 }}>Auto-detected mappings. Adjust if needed.</p>
      <p style={{ color: C.t2, marginBottom: 28, fontSize: 12, fontStyle: "italic" }}>Platform, Geo, Language, User Type, Placement, Asset Type, Ad Format → all auto-parsed from campaign names.</p>
      ...
      <div style={{ textAlign: "right" }}>
        <button onClick={onLaunch} style={{ background: C.p, color: "#fff", border: "none", borderRadius: 8, padding: "12px 28px", cursor: "pointer", fontFamily: C.font, fontSize: 14, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 8 }}>
...
function DashboardPage({ data, afData }) {
  ...
  // Aggregate totals
  const spend = sumK(filtered, "Spend");
  const imp = sumK(filtered, "Impressions");
  const clk = sumK(filtered, "Clicks");
  const cv = sumK(filtered, "Complete Views");
  const sess = sumK(filtered, "Sessions");
  const inst = sumK(filtered, "Installs");

  // Calculated metrics from totals
  const cpm = imp ? (spend / imp * 1000) : 0;
  const ctr = imp ? (clk / imp * 100) : 0;
  const vtr = imp ? (cv / imp * 100) : 0;
  const cpv = cv ? (spend / cv) : 0;
  const cpc = clk ? (spend / clk) : 0;
  const cps = sess ? (spend / sess) : 0;

  // Trend
  const byDate = useMemo(() => {
    const m = {};
    filtered.forEach(r => {
      if (!m[r.Date]) m[r.Date] = { Date: r.Date, Spend: 0, Impressions: 0, "Complete Views": 0, Clicks: 0 };
      const g = m[r.Date]; g.Spend += r.Spend || 0; g.Impressions += r.Impressions || 0; g["Complete Views"] += r["Complete Views"] || 0; g.Clicks += r.Clicks || 0;
    });
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: C.t0, marginBottom: 2 }}>Performance Dashboard</h1>
          <span style={{ fontSize: 12, color: C.t2 }}>{filtered.length.toLocaleString()} records</span>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {["7D", "14D", "30D", "All"].map(r => (
            <button key={r} onClick={() => setRange(r)} style={{ background: range === r ? C.p : "transparent", color: range === r ? "#fff" : C.t1, border: `1px solid ${range === r ? C.p : C.border}`, borderRadius: 20, padding: "5px 14px", cursor: "pointer", fontSize: 12, fontFamily: C.font, fontWeight: 500 }}>{r}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        {sess > 0 && <KpiCard label="Sessions" value={fmtN(sess)} color="#f472b6" />}
        {inst > 0 && <KpiCard label="Installs" value={fmtN(inst)} color={C.a} />}
      </div>

      {/* ── KPI ROW 2: Efficiency metrics ── */}
      <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        <KpiCard label="CPM" value={"₹" + fmtD(cpm)} sub="Cost / 1000 Impressions" color={C.a} />
        <KpiCard label="CTR" value={fmtD(ctr, 4) + "%"} sub="Clicks / Impressions" color={C.cy} />
        <KpiCard label="VTR" value={fmtD(vtr) + "%"} sub="Views / Impressions" color={C.g} />
        <KpiCard label="CPV" value={"₹" + fmtD(cpv, 4)} sub="Cost / Complete View" color={C.r} />
        ...
        {byDate.length > 1 ? (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={byDate}>
              ...
              <XAxis dataKey="Date" tick={{ fill: C.t2, fontSize: 10 }} tickFormatter={d => d.slice(5)} />
              <YAxis yAxisId="l" tick={{ fill: C.t2, fontSize: 10 }} tickFormatter={fmtN} />
              <YAxis yAxisId="r" orientation="right" tick={{ fill: C.t2, fontSize: 10 }} tickFormatter={fmtN} />
              <Tooltip contentStyle={tt} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Line yAxisId="l" type="monotone" dataKey="Spend" stroke={C.p} strokeWidth={2} dot={false} />
              <Line yAxisId="r" type="monotone" dataKey="Complete Views" stroke={C.g} strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        ) : <div style={{ color: C.t2, textAlign: "center", padding: 40 }}>Need 2+ dates for trend</div>}
      </div>

      {/* ── DIMENSION BREAKDOWNS 2×3 ── */}
      ...
      <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>
              {["Campaign", "Spend", "Impr", "Clicks", "Views", "CPM", "CTR", "VTR", "CPV", "CPC"].map(h => (
                <th key={h} style={{ padding: "10px 12px", textAlign: h === "Campaign" ? "left" : "right", fontSize: 10, fontWeight: 600, color: C.t2, textTransform: "uppercase", letterSpacing: .5, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {byCampaign.map((r, i) => (
                <tr key={i} style={{ background: i % 2 ? C.bg1 + "60" : "transparent" }}>
...
/* ═══════════════════════════════════════════════════════════
   PAGE: ANALYTICS BUILDER
   ═══════════════════════════════════════════════════════════ */
function AnalyticsPage({ data }) {
  const [dims, setDims] = useState(["Geo", "Placement"]);
  const [metrics, setMetrics] = useState(["Spend", "Complete Views", "VTR", "CPM", "CTR"]);
  ...
  const pivotData = useMemo(() => {
    ...
    data.forEach(r => {
      ...
      if (!m[key]) m[key] = { _key: key, Spend: 0, Impressions: 0, Clicks: 0, "Complete Views": 0, Sessions: 0, Installs: 0 };
      const g = m[key]; g.Spend += r.Spend || 0; g.Impressions += r.Impressions || 0; g.Clicks += r.Clicks || 0; g["Complete Views"] += r["Complete Views"] || 0; g.Sessions += r.Sessions || 0; g.Installs += r.Installs || 0;
    });
    return Object.values(m).map(g => ({
      ...g,
      CPM: g.Impressions ? (g.Spend / g.Impressions * 1000) : 0,
  ...
  const fmtCell = (v, m) => {
    if (["Spend"].includes(m)) return fmtC(v);
    if (["Impressions", "Clicks", "Complete Views", "Sessions", "Installs"].includes(m)) return fmtN(v);
    if (["CTR"].includes(m)) return fmtD(v, 4) + "%";
    if (["VTR"].includes(m)) return fmtD(v) + "%";
    if (["CPM", "CPC", "CPS"].includes(m)) return v ? "₹" + fmtD(v) : "—";
    if (m === "CPV") return v ? "₹" + fmtD(v, 4) : "—";
    return String(v);
  };
  ...
  return (
    <div>
      ...
      <p style={{ color: C.t2, marginBottom: 24, fontSize: 14 }}>Custom pivot tables with all parsed dimensions and calculated metrics.</p>
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
          ...
          {!dims.length ? <div style={{ color: C.t2, textAlign: "center", padding: 60 }}>Select a dimension</div> : (
            <div style={{ overflowX: "auto", maxHeight: 520, overflowY: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead style={{ position: "sticky", top: 0, background: C.bg2, zIndex: 2 }}>
                  <tr>
                    ...
                    {metrics.map(m => <th key={m} style={{ padding: "10px 14px", textAlign: "right", fontSize: 11, fontWeight: 600, color: C.t2, borderBottom: `1px solid ${C.border}`, textTransform: "uppercase", whiteSpace: "nowrap" }}>{m}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {pivotData.map((r, i) => (
                    <tr key={i} style={{ background: i % 2 ? C.bg1 + "60" : "transparent" }}>
                      <td style={{ padding: "10px 14px", fontSize: 12, color: C.t0, borderBottom: `1px solid ${C.border}15`, whiteSpace: "nowrap", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis" }} title={r._key}>{r._key}</td>
                      {metrics.map(m => <td key={m} style={{ padding: "10px 14px", fontSize: 12, color: C.t1, textAlign: "right", fontFamily: C.mono, borderBottom: `1px solid ${C.border}15`, whiteSpace: "nowrap" }}>{fmtCell(r[m], m)}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
...
function InsightsPage({ data, afData }) {
  const insights = useMemo(() => {
    const res = [];
    const ts = sumK(data, "Spend") || 1, tcv = sumK(data, "Complete Views") || 1, ti = sumK(data, "Impressions") || 1, tclk = sumK(data, "Clicks") || 1;
    const avgVTR = tcv / ti * 100, avgCPV = ts / tcv, avgCPM = ts / ti * 1000, avgCTR = tclk / ti * 100;

    const byGeo = aggByDim(data, "Geo").filter(g => g.name !== "Other");
    if (byGeo.length > 0) { const best = byGeo.reduce((a, b) => a.CPV < b.CPV ? a : b); res.push({ type: "success", title: `${best.name} — best CPV at ₹${fmtD(best.CPV, 4)}`, desc: `VTR ${fmtD(best.VTR)}%, CPM ₹${fmtD(best.CPM)}, CTR ${fmtD(best.CTR, 4)}%. ${fmtD(best.Spend / ts * 100, 0)}% of budget.`, action: `Scale ${best.name} budget 15-20%.` }); }
    if (byGeo.length > 1) { const worst = byGeo.reduce((a, b) => a.CPV > b.CPV ? a : b); if (worst.CPV > avgCPV * 1.3) res.push({ type: "error", title: `${worst.name} — high CPV ₹${fmtD(worst.CPV, 4)}`, desc: `${fmtD((worst.CPV - avgCPV) / avgCPV * 100, 0)}% above average. VTR ${fmtD(worst.VTR)}%, CTR ${fmtD(worst.CTR, 4)}%.`, action: `Reduce ${worst.name} allocation 20-30% or refresh creatives.` }); }

    const byPlace = aggByDim(data, "Placement").filter(p => p.name !== "Other");
    if (byPlace.length >= 2) { const best = byPlace.reduce((a, b) => a.VTR > b.VTR ? a : b); const worst = byPlace.reduce((a, b) => a.VTR < b.VTR ? a : b); res.push({ type: "info", title: `${best.name}: ${fmtD(best.VTR)}% VTR vs ${worst.name}: ${fmtD(worst.VTR)}%`, desc: `CPV: ${best.name} ₹${fmtD(best.CPV, 4)} vs ${worst.name} ₹${fmtD(worst.CPV, 4)}. CTR: ${fmtD(best.CTR, 4)}% vs ${fmtD(worst.CTR, 4)}%.`, action: `Prioritize ${best.name} for awareness. Use ${worst.name} for reach.` }); }

    const byLang = aggByDim(data, "Language").filter(l => l.name !== "Other");
    if (byLang.length >= 2) { const best = byLang.reduce((a, b) => a.VTR > b.VTR ? a : b); res.push({ type: "info", title: `${best.name} content leads with ${fmtD(best.VTR)}% VTR`, desc: `CPM ₹${fmtD(best.CPM)}, CPV ₹${fmtD(best.CPV, 4)}, CTR ${fmtD(best.CTR, 4)}%.`, action: `Invest in more ${best.name} creative variants.` }); }

    const byUser = aggByDim(data, "UserType").filter(u => u.name !== "Unknown");
    if (byUser.length >= 2) { const nu = byUser.find(u => u.name === "New User"), ru = byUser.find(u => u.name === "Returning User"); if (nu && ru) res.push({ type: nu.CPV < ru.CPV ? "success" : "warning", title: `New Users CPV ₹${fmtD(nu.CPV, 4)} vs Returning ₹${fmtD(ru.CPV, 4)}`, desc: `NU: VTR ${fmtD(nu.VTR)}%, CTR ${fmtD(nu.CTR, 4)}%. RU: VTR ${fmtD(ru.VTR)}%, CTR ${fmtD(ru.CTR, 4)}%.`, action: nu.CPV < ru.CPV ? "Maintain NU acquisition focus." : "Consider increasing RU retargeting." }); }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div><h1 style={{ fontSize: 24, fontWeight: 700, color: C.t0, marginBottom: 2 }}>AI Insights Engine</h1><p style={{ color: C.t2, fontSize: 14 }}>Automated performance diagnosis.</p></div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {insights.map((ins, i) => (
          <div key={i} style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, borderLeft: `4px solid ${colMap[ins.type]}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}><div style={{ width: 12, height: 12, borderRadius: 6, background: colMap[ins.type] }} /><span style={{ fontWeight: 600, fontSize: 14, color: C.t0 }}>{ins.title}</span></div>
...
function ReportsPage({ data, afData }) {
  ...
  const dates = [...new Set(data.map(r => r.Date))].sort();
  const dr = dates.length ? `${dates[0]} to ${dates[dates.length - 1]}` : "—";
...
—— RECOMMENDATIONS ——
1. Scale best CPV regions
2. Optimize creative mix by language
3. Monitor VTR & CTR trends weekly`,

    weekly: `📧 WEEKLY PERFORMANCE — ${dr}

💰 Spend: ${fmtC(s)}
👁 Impressions: ${fmtN(imp)} | 🖱 Clicks: ${fmtN(clk)} | ▶ Views: ${fmtN(cv)}
📈 CPM: ₹${fmtD(cpm)} | CTR: ${fmtD(ctr, 4)}% | VTR: ${fmtD(vtr)}%
🎬 CPV: ₹${fmtD(cpv, 4)} | CPC: ₹${fmtD(cpc)}${sess ? ` | CPS: ₹${fmtD(cps)}` : ""}${inst ? `\n📲 Installs: ${fmtN(inst)}${purch ? ` | Purchases: ${fmtN(purch)}` : ""}` : ""}`,

    executive: `📋 EXECUTIVE SUMMARY — ${dr}

Budget: ${fmtC(s)} | Impressions: ${fmtN(imp)} | Views: ${fmtN(cv)}
CPM: ₹${fmtD(cpm)} | CTR: ${fmtD(ctr, 4)}% | VTR: ${fmtD(vtr)}% | CPV: ₹${fmtD(cpv, 4)}
${inst ? `Installs: ${fmtN(inst)}${purch ? ` | Purchases: ${fmtN(purch)}` : ""}` : ""}

Video completions ${vtr > 70 ? "strong" : "need optimization"} at ${fmtD(vtr)}% VTR.`,
  };

  const handleCopy = () => {
    navigator.clipboard.writeText(reports[tab]).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }).catch(() => {
      const t = document.createElement("textarea"); t.value = reports[tab]; document.body.appendChild(t); t.select(); document.execCommand("copy"); document.body.removeChild(t); setCopied(true); setTimeout(() => setCopied(false), 2000);
    });
  };
  ...
  return (
    <div>
      ...
      <p style={{ color: C.t2, marginBottom: 24, fontSize: 14 }}>Auto-generated with your actual KPIs.</p>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", gap: 8 }}>
          {[{ id: "completion", l: "Completion" }, { id: "weekly", l: "Weekly" }, { id: "executive", l: "Executive" }].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ background: tab === t.id ? C.p : "transparent", color: tab === t.id ? "#fff" : C.t1, border: `1px solid ${tab === t.id ? C.p : C.border}`, borderRadius: 8, padding: "8px 20px", cursor: "pointer", fontSize: 13, fontFamily: C.font, fontWeight: 500 }}>{t.l}</button>
          ))}
        </div>
        <button onClick={handleCopy} style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 18px", cursor: "pointer", color: copied ? C.g : C.t1, fontFamily: C.font, fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
          {copied ? <CheckCircle2 size={14} /> : <Copy size={14} />}{copied ? "Copied!" : "Copy"}
        </button>
      </div>
      <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 32 }}>
        <pre style={{ fontFamily: C.mono, fontSize: 13, color: C.t1, lineHeight: 1.7, whiteSpace: "pre-wrap", margin: 0 }}>{reports[tab]}</pre>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const handleFile = useCallback(async f => {
    if (!f) return;
    setMediaFile(f.name); setData(null); setValResult(null); setRawRows(null); setHeaders([]);
    try {
      ...
      setValResult(vr.errs.length || vr.warns.length ? vr : null);
    } catch (e) { setValResult({ errs: [e.message], warns: [], clean: [] }); }
  }, []);

  const handleAfFile = useCallback(async f => {
    if (!f) return;
    setAfFile(f.name);
    try {
      const { headers: h, rows } = await parseFile(f);
    ...
    } catch (e) { console.warn("AF error:", e.message); }
  }, []);

  const handleDemo = useCallback(() => {
    const demo = [
      { Date: "2026/03/12", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_HINDI-CITIES(MH & DNCR ONLY)_AWA_INNS-CTV", "YouTube Ad": "HM_SWIGGY_INNS-CTV_RS9_15SEC_HIN", "Revenue (Adv Currency)": "22570.66", Impressions: "295536", Clicks: "116", "Complete Views (Video)": "260507" },
      { Date: "2026/03/12", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_ENGLISH-GEOS_AWA_INNS-CTV", "YouTube Ad": "HM_SWIGGY_INNS-CTV_RS9_15SEC_ENG", "Revenue (Adv Currency)": "23990.66", Impressions: "279788", Clicks: "89", "Complete Views (Video)": "245829" },
      { Date: "2026/03/13", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_TAMILNADU_AWA_VVC-CTV", "YouTube Ad": "HM_SWIGGY_SHORTS_RS9_15SEC_TAM", "Revenue (Adv Currency)": "1800", Impressions: "28000", Clicks: "45", "Complete Views (Video)": "11200" },
      { Date: "2026/03/13", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_KERALA_AWA_INNS-CTV", "YouTube Ad": "HM_SWIGGY_INNS-CTV_RS9_15SEC_MAL", "Revenue (Adv Currency)": "3426.89", Impressions: "49457", Clicks: "16", "Complete Views (Video)": "40380" },
      { Date: "2026/03/14", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_ANDHRA PRADESH/TELANGANA_AWA_VVC-CTV", "YouTube Ad": "HM_SWIGGY_SKIP_RS9_15SEC_TEL", "Revenue (Adv Currency)": "2967.68", Impressions: "49766", Clicks: "58", "Complete Views (Video)": "27486" },
      { Date: "2026/03/14", "Insertion Order": "HM_SWIGGY_DV360_VIDEO_YT_RS9_NU_HINDI-CITIES(EX-MH-DNCR)_AWA_INNS-CTV", "YouTube Ad": "HM_SWIGGY_INNS-CTV_RS9_15SEC_HIN", "Revenue (Adv Currency)": "27946.40", Impressions: "403885", Clicks: "184", "Complete Views (Video)": "354564" },
    ];
    const h = Object.keys(demo[0]); setHeaders(h); setRawRows(demo);
    const am = autoMap(h); setColMap(am); setMediaFile("demo-dv360.csv");
    const vr = validate(demo, am, "media");
    setData(vr.clean); setValResult(null); setTab("dashboard");
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg,${C.bg0} 0%,#0d1222 40%,${C.bg0} 100%)`, color: C.t0, fontFamily: C.font, fontSize: 14 }}>
