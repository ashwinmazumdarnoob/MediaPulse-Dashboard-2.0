...
function isDV360Footer(row) {
  ...
  const first = String(vals[0]?.[1]||"").toLowerCase().trim();
  // Known DV360 footer prefixes
  if (/^(date range|group by|filter by|mrc accredited|reporting numbers|report time)/.test(first)) return true;
  // Row where first cell is empty and has very few non-empty values (metadata)
  if (first===""&&Object.values(row).filter(x=>String(x||"").trim()&&String(x).toLowerCase()!=="undefined").length<=1) return true;
  // DV360 grand total row: Date is empty AND Campaign is empty BUT has numeric data
  // These are summary rows that would double all metrics
  const dateVal = String(row.Date||row.date||vals[1]?.[1]||"").trim();
  const campVal = String(row["Insertion Order"]||row.Campaign||row.campaign||vals[2]?.[1]||"").trim();
  if (!dateVal && !campVal) {
    // Check if any other cell has a large number (summary indicator)
    const hasLargeNum = Object.values(row).some(v => {
      const n = parseFloat(String(v||"").replace(/,/g,""));
      return !isNaN(n) && n > 10000;
    });
    if (hasLargeNum) return true;
  }
  // Row with "undefined" string values (DV360 Report Time row)
  const undefinedCount = Object.values(row).filter(v=>String(v).toLowerCase()==="undefined").length;
  if (undefinedCount >= 3) return true;
  return false;
}
...
function parseNum(v) {
  ...
  let s = String(v).trim();
  // Reject known non-numeric strings immediately
  if (!s || s==="—" || s==="-" || s==="N/A" || s==="null" || s==="undefined" || s==="NaN" || s==="#N/A" || s==="#VALUE!" || s==="#DIV/0!") return 0;
  // Strip currency symbols, commas, quotes, whitespace, percent signs
...
function validate(rows,cm,type){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    const vals=Object.values(raw).filter(v=>v!=null&&String(v).trim()&&String(v).toLowerCase()!=="undefined");if(!vals.length){blank++;continue;}
    const campVal=String(raw[cm.Campaign]||"").trim();
    const dateRaw=String(raw[cm.Date]||"").trim();
    // Skip rows where BOTH date and campaign are empty — these are summary/total rows
    if(!dateRaw&&!campVal){stripped++;continue;}
    // Skip rows with total/sum/grand in campaign name
    if(/^(total|sum|grand|subtotal|average)\b/i.test(campVal)){stripped++;continue;}
    // Skip rows where campaign is "undefined" (DV360 Report Time artifact)
    if(campVal.toLowerCase()==="undefined"){stripped++;continue;}
    const row={_i:i+2};const dv=parseDate(dateRaw);
    // Skip rows with unparseable dates (not YYYY-MM-DD after parsing) — they corrupt aggregations
    if(!/^\d{4}-\d{2}-\d{2}$/.test(dv)){badDate++;if(badDate<=2)warns.push(`Row ${i+2}: Date "${dateRaw}" skipped (unparseable).`);continue;}
